...
#function to get the digit clicked on the calculator
def get_digit(digit):
    current = result_label['text']
    new = current + str(digit)
    result_label.config(text=new)

#function to get the operator clicked on the calculator
def get_operator(op):
...
def get_result():
    ...
    if operator == '+':
        ...
        if second_number == 0:
            ...
            result_label.config(text=round(first_number/second_number,2))
...
#creating label to display the result
...
btn_subs = Button(root,text='-',bg='#00a65a',fg='white',width=5,height=2,command=lambda : get_operator('-'))
btn_subs.grid(row=2,column=3)
btn_subs.config(font=('verdana',14))
